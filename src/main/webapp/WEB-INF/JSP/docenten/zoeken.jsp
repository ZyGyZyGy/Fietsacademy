...
<body>
	...
	<form>
		...
		<input type='number' name='id' value='${param.id}' required autofocus min='1'></label>
    ...
    ${docent.naam}, wedde: &euro; <fmt:formatNumber value='${docent.wedde}' />
		<c:if test='${not empty docent.bijnamen}'>
			...
			<ul>
				<c:forEach items='${docent.bijnamen}' var='bijnaam'>
					<li>
						<label>${bijnaam} 
						<input type='checkbox' name='bijnaam' value='${bijnaam}'></label>
					</li> 
				</c:forEach>
			</ul>
		...
		<form method='post' id='toevoegform'>
			...
			<input type="text" name='bijnaam' value='${param.bijnaam}' required></label> 
